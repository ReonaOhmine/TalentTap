...
<div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
  <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 dark:bg-neutral-800 dark:border-neutral-700">
    <h2 class="text-xl font-semibold text-gray-800 dark:text-neutral-200 mb-6">ユーザー編集</h2>
    <form action="{{ route('agent.customer.update', $user->id) }}" method="POST">
      ...
      <div class="grid gap-6 mb-6 lg:grid-cols-2">
        <div>
          ...
          <input type="text" id="name" name="name" value="{{ $user->name }}" class="block w-full p-2.5 border border-gray-300 rounded-lg shadow-sm dark:bg-neutral-700 dark:border-neutral-600 dark:text-white dark:placeholder-gray-400" required>
        </div>
        <div>
          <label for="position" class="block mb-2 text-sm font-medium text-gray-900 dark:text-neutral-200">ポジション</label>
          <input type="text" id="position" name="position" value="{{ $user->position }}" class="block w-full p-2.5 border border-gray-300 rounded-lg shadow-sm dark:bg-neutral-700 dark:border-neutral-600 dark:text-white dark:placeholder-gray-400" required>
        </div>
        <div>
        ...
        <div>
          ...
          <input type="number" id="matching" name="matching" value="{{ $user->matching }}" class="block w-full p-2.5 border border-gray-300 rounded-lg shadow-sm dark:bg-neutral-700 dark:border-neutral-600 dark:text-white dark:placeholder-gray-400" min="0" max="5" required>
        </div>
        <div>
          <label for="created_at" class="block mb-2 text-sm font-medium text-gray-900 dark:text-neutral-200">作成日</label>
          <input type="datetime-local" id="created_at" name="created_at" value="{{ $user->created_at->format('Y-m-d\TH:i') }}" class="block w-full p-2.5 border border-gray-300 rounded-lg shadow-sm dark:bg-neutral-700 dark:border-neutral-600 dark:text-white dark:placeholder-gray-400" required>
        </div>
      </div>
